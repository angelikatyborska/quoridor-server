  ...
  class Lobby
    ...
    def create_room(player, data)
      unless data['capacity']
        fail Quoridor::MalformedMessage.new(data, 'capacity cannot be blank')
      end

      room = Quoridor::Room.new(player, data['capacity'].to_i)
      @rooms << room
    ...
    def join_room(player, data)
      unless room = @rooms.find { |room| room.id == data['room_id'] }
        fail Quoridor::MalformedMessage.new(data, "room with id #{data['room_id']} not found")
      end

      if @players_ids_to_rooms_ids[player.id]
        fail Quoridor::MalformedMessage.new(data, "player already inside room #{@players_ids_to_rooms_ids[player.id].id}")
      end
    ...
    def leave_room(player, data)
      unless room = @rooms.find { |room| room.id == data['room_id'] }
        fail Quoridor::MalformedMessage.new(data, "room with id #{room.id} not found")
      end

      unless @players_ids_to_rooms_ids[player.id] == room.id
        fail Quoridor::MalformedMessage.new(data, "player is not inside room #{data['room_id']}}")
      end
